...
DB_PATH = DATA_DIR / "flight_optimizer.db"
...
def load_all_offers():
    ...
    with db_connect() as conn:
        rows = conn.execute(
            "SELECT payload_json FROM offers ORDER BY id"
        ).fetchall()
    for row in rows:
        try:
...
if "offers" not in st.session_state:
    st.session_state.offers = load_all_offers()
if "generated" not in st.session_state:
    st.session_state.generated = load_state("generated", None)
...
with st.sidebar:
    st.markdown("### 💾 자동저장")
    st.success("SQLite 자동저장 ON")
    st.caption(f"DB 파일: {DB_PATH.name}")
    st.caption("앱 종료/PC 재부팅 후에도 여행 설정과 저장 항공편을 자동 복구합니다.")

    backups = sorted(BACKUP_DIR.glob("flight_optimizer_*.db"), reverse=True)
    if backups:
        st.caption(f"최근 백업: {backups[0].name}")

    if st.button("지금 백업 만들기"):
...
st.caption("입력 데이터는 data/flight_optimizer.db에 자동저장되며 backup/ 폴더에 일일 백업됩니다.")
